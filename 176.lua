...
-- Function to check if a player has "user" in any BillboardGui
local function hasUserInBillboard(character)
    if not character then return false end
    
    -- Check all descendants for BillboardGui
    for _, descendant in ipairs(character:GetDescendants()) do
        if descendant:IsA("BillboardGui") then
            -- Check all TextLabels and TextButtons in the BillboardGui
            for _, child in ipairs(descendant:GetDescendants()) do
                if child:IsA("TextLabel") or child:IsA("TextButton") then
                    local text = child.Text:lower()
                    if text:find("user") then
                        return true
                    end
                end
            end
...
-- Get the closest player (excluding those with "user" in BillboardGui)
local function getClosestPlayer()
    ...
    for _, other in ipairs(Players:GetPlayers()) do
        if other ~= player and other.Character then
            -- Skip players with "user" in their BillboardGui
            if not hasUserInBillboard(other.Character) then
